...
class UnitPriority(enum.Enum):
    New = 0,
    Idle = 1,
    Defending = 2, # TODO   defending what ? some method to link Role with expansions or key locations
    Attacking = 3, # TODO   Attacking what ? some method to link Role with expansions or key locations
    Harassing = 4, # TODO   Harassing what ? some method to link Role with expansions or key locations
    Scouting = 4,  # TODO   Scouting what ? some method to link Role with expansions or key locations
...
class BaseDriver:
    ...
    def seek_and_harras(self, unit: Unit) -> bool:  # for harrasing not sure if this should return bool or not, or anything
        ...
        if len(enemy_workers) > 0:
            ...
            try:
                loc = random.choice(self.ai.enemy_start_locations) \
                    .towards_with_random_angle(self.ai.game_info.map_center,
                                               distance=5)
                self.ai.do(unit.move(loc))
            except Exception as e:
                print(e)
                print("this is not a ladder map,  adjusting...")
                loc = random.choice(list(self.ai.expansion_locations.keys())) \
                    .towards_with_random_angle(self.ai.game_info.map_center,
                                               distance=5)
                self.ai.do(unit.move(loc))
